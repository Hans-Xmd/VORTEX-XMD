let handler = async (message, { conn, text }) => {
  ...
  const creatorName = "Hans The";
  const creatorTitle = "Techie (HansTz Devs) 🧑‍💻";
  const support = "Support:";
  const supportLink = "https://www.google.com/search?q=HANSTZTECH";
  ...
  if (global.db.data.users[senderId].lover === '') {
    ...
    conn.reply(
      message.chat,
      `*HansTz says: You have no partner and are not loving anyone.*\n*Type /propose @user to propose to someone.*\n\n${footerMessage}`,
      message
    );
  } else {
    ...
    if (global.db.data.users[global.db.data.users[senderId].lover].lover !== senderId) {
      ...
      conn.reply(
        message.chat,
        `*Vortex says: You are in a relationship with ${partnerName} 🥰☺️*\n\n${footerMessage}`,
        message,
        { mentions: [partnerId] }
      );
    }
